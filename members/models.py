...
class AtividadeParceiro(models.Model):
...
class Parceiros(models.Model):
    ...
    atividades = models.ManyToManyField(AtividadeParceiro, related_name='parceiros', blank=True)
...
class PlanosCondPagto(models.Model):
    ...
    forma_pagamento = models.CharField(max_length=21, choices=FormaDePagamento.choices, default=FormaDePagamento.BOLETO)
    ...
    taxa_desconto_parcelas = models.DecimalField(max_digits=5, decimal_places=2)
    ativo = models.BooleanField(default=True)
    data_cadastro = models.DateTimeField(auto_now_add=True)
    data_atualizacao = models.DateTimeField(auto_now=True)  
...
class MembroNegociacao(models.Model):
    ...
    formpagto = models.CharField(max_length=21, choices=FormaDePagamento.choices, default=FormaDePagamento.BOLETO)
    ...
    def __str__(self):
        return f"{self.membro} - R$ {self.valor_negociado} - {self.dataemissao.strftime('%d/%m/%Y')}"
